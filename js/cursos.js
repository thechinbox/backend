...
var GETCURSOSPRO = function (req, res) {
    var cursos = new Array();
    pool.query('SELECT cu.clavecurso, CONCAT(pro.nombres, \' \', pro.apellidos) as profesor, nombrecurso, descripcion FROM curso cu INNER JOIN profesional pro ON cu.rutpro = pro.rutpro  WHERE publicado = $2 AND cerrado = false AND cu.rutpro = $1', [req.body.rut, req.body.publicado], function (err, resp) {
        if (err) {
            console.error(err);
            return;
        }
            ...
            for (var _i = 0, _a = resp.rows; _i < _a.length; _i++) {
                var row = _a[_i];
                var modulos = new Array();
                var curso_2 = { "clavecurso": row.clavecurso, "profesor": row.profesor, "nombrecurso": row.nombrecurso, "descripcion": row.descripcion, "modulos": modulos };
                cursos.push(curso_2);
            }
        }
        res.send(JSON.stringify(cursos));
