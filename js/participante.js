    ...
    pool.query('SELECT cu.clavecurso, CONCAT(pro.nombres, \' \', pro.apellidos) as profesor, nombrecurso, descripcion FROM curso cu INNER JOIN profesional pro ON cu.rutpro = pro.rutpro  INNER JOIN participante pa ON (pa.rutcomun = $1 AND pa.clavecurso = cu.clavecurso AND pa.finalizado = false)', [req.body.rut], function (err, resp) {
        if (err) {
            ...
            for (var _i = 0, _a = resp.rows; _i < _a.length; _i++) {
                var row = _a[_i];
                var curso_1 = { "clavecurso": row.clavecurso, "profesor": row.profesor, "nombrecurso": row.nombrecurso, "descripcion": row.descripcion, "modulos": new Array() };
                cursos.push(curso_1);
            }
        }
        ...
        if (err) {
            ...
            for (var _i = 0, _a = resp.rows; _i < _a.length; _i++) {
                ...
                if (index_modulo == -1) {
                    ...
                    curso.modulos.push({ "id": row.idmodulo, "nombre": row.nombremodulo, "nromodulo": row.nromodulo_curso, "descripcion": row.descripcionmodulo, "video": row.video, "clases": new Array() });
                    index_modulo += 1;
